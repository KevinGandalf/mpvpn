...
for entry in "${EXTRA_RT_TABLES[@]}"; do
    ...
    # Wenn die Tabelle "streaming" ist, soll der Verkehr direkt über das Gateway gehen
    if [[ "$table_name" == "streaming" ]]; then
        # Regel für den gesamten Verkehr der Tabelle 100, der direkt über den Router gehen soll
        echo "Routing für 'streaming' geht direkt über den Router..."
