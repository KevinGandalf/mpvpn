...
if [[ "$ENABLE_UNBOUND" == "true" ]]; then
    ...
    bash $BASE_PATH/helperscripts/addons/dns/install_unbound.sh
else
...
if [[ "$ENABLE_DNSCRYPT" == "true" ]]; then
    ...
    bash $BASE_PATH/helperscripts/addons/dns/install_dnscrypt.sh
else
...
if [[ "$ENABLE_UNBOUND" == "true" && "$ENABLE_DNSCRYPT" == "true" ]]; then
    ...
    bash $BASE_PATH/helperscripts/addons/dns/configure_dns_chain.sh
else
